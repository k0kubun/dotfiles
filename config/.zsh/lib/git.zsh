...
function gl(){
	...
	if [ $# -ne 0 ]; then
		git --no-pager log --date=iso --no-show-signature --pretty=format:'%Cgreen%h %Cblue%an %Creset%s %C(blue)%d%Creset' "$@"
	else
		git --no-pager log --date=iso --no-show-signature --pretty=format:'%Cgreen%h %Cblue%an %Creset%s %C(blue)%d%Creset' -10
	fi
}
